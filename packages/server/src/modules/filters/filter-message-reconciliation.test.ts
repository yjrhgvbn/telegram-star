...
describe("planFilterMessageReconciliation", () => {
  it("rechecks confirmed user identities without treating legacy sender IDs as users", async () => {
    const messages = [
    ...
    expect((await planFilterMessageReconciliation(messages, [{ type: "sender", values: ["123"] }]))).toEqual({
      messageIdsToDelete: [2, 3],
      keywordUpdates: [{ messageIds: [1], matchedKeyword: null }],
    });
    expect((await planFilterMessageReconciliation(messages, [{ type: "sender", effect: "exclude", values: ["123"] }])).messageIdsToDelete).toEqual([1]);
  });

  it("removes messages that no longer satisfy every updated condition", async () => {
    const plan = (await planFilterMessageReconciliation(
      [
  ...
  it("clears a stale keyword when the updated rule only restricts chats", async () => {
    const plan = (await planFilterMessageReconciliation(
  ...
  it("does not silently delete history when a custom script throws", async () => {
    await expect(
