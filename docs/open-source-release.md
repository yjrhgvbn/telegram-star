...
- MIT LICENSE、贡献说明、PR 自动运行及可手动复用的 CI 检查。
...
4. **验证首次安装。** PR 运行 CI，必要时也可手动运行；版本 tag 会触发发布工作流自身的构建、测试及类型检查。检查通过后，按 Release 的 Docker 命令用空数据卷验证首次安装，分别检查不设置密码和启用密码的访问流程，并试装客户端。普通 build 不生成原生安装包，工作流配置也不代表已有下载产物。
5. **检查触发与保护规则。** PR 自动运行 CI，分支保护可要求对应检查成功；普通分支推送不单独触发 CI。`main` 推送触发自用服务器的 SSH 源码部署，先复用 CI 并部署同一 SHA，服务器工作区有本地修改时停止。该流程和 tag 发布镜像、安装包互相独立；发布 Demo 不需要 SSH / Telegram Secrets。
...
7. 维护者自己的服务器由 `main` 推送或手动 **Deploy To Server** 验证并部署该次运行的相同提交，在干净的服务器工作区本地构建镜像；无需等待公开版本发布。SSH 配置见[部署到自己的服务器](version-releases.md#部署到自己的服务器)。普通用户复制 Release 的 Docker 命令启动服务，需要客户端时下载安装包。
...
- 后台密码可选，任意监听地址及 Docker 均可留空；非空值至少 16 字符。未启用时，可连接服务的人能够读取消息和修改配置。默认沿用 `0.0.0.0:3000` 入口，无需新增配置；仅本机使用时可主动绑定 `127.0.0.1`，远程传输凭据使用 HTTPS；后台密码与 Telegram 授权独立，不提供多用户隔离。
